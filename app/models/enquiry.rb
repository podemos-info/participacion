class Enquiry < ActiveRecord::Base
  ...
  validates :responsible_name, length: { in: 6..Enquiry.responsible_name_max_length }
  ...
  pg_search_scope :pg_search, {
    ...
    ranked_by: '(:tsearch + enquiries.cached_votes_up)',
  ...
  cattr_accessor :opened_enquiry_date
  @@opened_enquiry_date =DateTime.parse(Rails.application.secrets.opened_enquiry_date)
  ...
  def self.is_closed?
    ...
    return (DateTime.now.to_s < @@opened_enquiry_date.to_s or DateTime.now.to_s >= @@closed_enquiry_date.to_s)
  end
  ...
  def code
    "#{Setting.value_for("enquiry_code_prefix")}-#{created_at.strftime('%Y-%M')}-#{id}"
  end
  ...
  def after_hide
    self.tags.each{ |t| t.decrement_custom_counter_for('Enquiry') }
  end

  def after_restore
    self.tags.each{ |t| t.increment_custom_counter_for('Enquiry') }
  end
  ...
  def self.votes_needed_for_success
    Setting.value_for('votes_for_enquiry_success').to_i
  end
