...
class Law < ActiveRecord::Base
  ...
  validates :title, length: { in: 4..Law.title_max_length }
  validates :description, length: { in: 10..Law.description_max_length }
  ...
  def editable?
    total_votes <= Setting.value_for('max_votes_for_law_edit').to_i
  end
  ...
  def register_vote(user, vote_value)
    if votable_by?(user)
      Law.increment_counter(:cached_anonymous_votes_total, id) if (user.unverified? && !user.voted_for?(self))
  ...
  def votable_by?(user)
    total_votes <= 100 ||
      !user.unverified? ||
      Setting.value_for('max_ratio_anon_votes_on_laws').to_i == 100 ||
      anonymous_votes_ratio < Setting.value_for('max_ratio_anon_votes_on_laws').to_i ||
      user.voted_for?(self)
  end
  ...
  def self.search(terms)
    ...
    law_ids = where("laws.title ILIKE ? OR laws.description ILIKE ?",
                       "%#{terms}%", "%#{terms}%").pluck(:id)
    tag_ids = tagged_with(terms, wild: true, any: true).pluck(:id)
    where(id: [law_ids, tag_ids].flatten.compact)
  end

  def after_hide
    self.tags.each{ |t| t.decrement_custom_counter_for('Law') }
  end

  def after_restore
    self.tags.each{ |t| t.increment_custom_counter_for('Law') }
  end
